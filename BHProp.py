...
def ItauFI(tl, v, mDM, mX): 
    '''
    Freeze In case (Including mediator)
    '''
...
def ItauRH(tl, v, M1, M2, M3):
    '''
    Including 3 RH neutrinos
    '''
...
# Time as function of the log@ scale factor  --> in GeV^-1

def t_a(aexp, rPBHi, rRadi):

    a = 10.**(aexp)   
    A = np.sqrt(GCF * (rPBHi + rRadi))
    B = np.sqrt(GCF * (a * rPBHi + rRadi))    
    D = GCF * np.sqrt(6.*np.pi) * rPBHi**2

    if rPBHi/(rPBHi+rRadi) > 1.e-10:
        t = (rPBHi*(a*B - A) + 2.*rRadi*(A - B))/D
    else:
        t = 0.25*(a**2-1)*np.sqrt(3./(2.*np.pi*GCF*rRadi))
